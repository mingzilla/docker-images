...
- [Skill integration] External projects access ai-agents skills via symlink: `.claude/skills__central` → ai-agents skills dir. Set up via `system__manage_skill action__symlink_to_central_skills`. Symlink is gitignored. `system__save_session` must be guarded in CLAUDE.md — it saves to ai-agents, not the target project. Named `skills__central` (not `skills`) so the project can still have local `.claude/skills/` without collision.
...
- [User preferences] When a bash script has a configurable value that's reused in multiple places, extract it as a variable at the top of the file (e.g. `SKILLS_TARGET_NAME=".claude/skills__central"`) so it's easy to find and change without digging into the script.
- [Skill integration] `project__capture_decisions` set up for docker-images. Decision topics: `architecture`, `model-selection`, `infrastructure`, `skill-integration`, `conventions`. Coding preferences stay in learnings, not decisions.
